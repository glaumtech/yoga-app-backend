...
//        }
//
//        // 3️⃣ Fetch existing assignments for this event
//        Map<Long, Set<String>> participantCategoryMap = assignedRepo
//                .findByEventId(req.getEventId())
//                .stream()
...
//
//            if (assignedCategories.stream().anyMatch(c -> c.trim().equalsIgnoreCase(req.getCategory().trim()))) {
//                duplicates.add("ParticipantID:" + participant.getId() + " already assigned category: " + req.getCategory());
//                continue; // skip this assignment
//            }
//
//
//
//            String key = participant.getId() + "-" + jury.getId();
//
//
...
//
//                // Update map to avoid duplicates in same request
//                assignedCategories.add(req.getCategory());
//                participantCategoryMap.put(participant.getId(), assignedCategories);
//            }
//        }
//
////        // 5️⃣ Throw error if any duplicates were found
//        if (!duplicates.isEmpty()) {
//            throw new RuntimeException("Duplicate assignments detected: " + duplicates);
//        }
//
...
// 4️⃣ Prepare to save new assignments
    ...
    for (ParticipantsDto participant : req.getParticipants()) {
        ...
        // Check duplicate only against existing database assignments
//        if (assignedCategories.stream().anyMatch(c -> c.trim().equalsIgnoreCase(req.getCategory().trim()))) {
//            duplicates.add("ParticipantID:" + participant.getId() + " already assigned category: " + req.getCategory());
...
// 5️⃣ Throw error if any duplicates were found (against existing DB only)
//    if (!duplicates.isEmpty()) {
//        throw new RuntimeException("Duplicate assignments detected: " + duplicates);
//    }
//    if (!duplicates.isEmpty()) {
//        List messages = duplicates.entrySet().stream()
//                .map(e -> "Category '" + e.getKey() + "' is already assigned for participant(s) " + e.getValue().stream().map(String::valueOf).collect(Collectors.joining(",")))
//                .toList();
